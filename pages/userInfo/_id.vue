<template>
  <div class="user-detail-info-box  clear-fix default-border-radius">
    ...
    <div class="user-info-right-part float-left" id="user-info-right-part">
      <el-form label-width="86px" v-if="userInfo!==null" size="medium">
        <div class="info-item">
          <span class="info-left">昵称：
            <a><span>{{userInfo.userName}}</span></a>
            </span>
        </div>
        <div class="info-item">
        ...
        <div class="info-item">
          <span class="info-left clear-fix">邮箱：
        ...
        <div class="info-item sign-input clear-fix" v-else>
          ...
          <div class="float-left">
            <el-input v-model="sobUser.sign" type="text" placeholder="独一无二的签名！" size="medium"></el-input>
          </div>
        </div>
        <div class="info-item user-name-input clear-fix" v-if="isEditorMode">
          <div class="info-left float-left">昵称：</div>
          <div class="float-left">
            <el-input type="text" v-model="sobUser.userName" placeholder="万中无一的昵称！" size="medium"></el-input>
          </div>
        </div>
        <div class="update-btn" v-if="isEditorMode">
          <el-button size="medium" type="primary" @click="updateUserInfo">更新</el-button>
        </div>
        <div class="user-info-editor-btn" v-if="parUserId===currentUserId">
          <el-button size="medium" :type="isEditorMode?'danger':'primary'" @click="switchEditorMode"
...
<script>
  ...
  export default {
    head() {
      return {
        title: '本是人间红尘客-' + this.userInfo.userName + '用户信息',
        meta: [
          {
            hid: 'description',
            name: 'description',
            content: '本是人间红尘客,用户信息中心'
          },
          {
            hid: 'keywords',
            name: 'keywords',
    ...
    methods: {
      ...
      async updateUserInfo() {
        ...
        api.updateUserInfo(this.sobUser, this.sobUser.id).then(result => {
          if (result.code === api.success_code) {
