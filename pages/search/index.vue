  ...
  <div class="search-box" id="search-box">
      ...
      <div class="search-condition-box">
        <div class="select-item">
          <span
            ...
            @click="doSearchBySort('')"
            >综合排序</span
          ><span
            @click="doSearchBySort('2')"
            :class="sort === '2' || sort === '1' ? 'sort-active' : ''"
            >时间</span
          ><span
            @click="doSearchBySort('4')"
            :class="sort === '3' || sort === '4' ? 'sort-active' : ''"
            >浏览量</span
          >
        </div>
        <div class="select-item">
          <span
            ...
            @click="doSearchByCategory('')"
            >全部分类</span
          ><span
            @click="doSearchByCategory(item.id)"
            :class="categoryId === item.id ? 'category-active' : ''"
    ...
    <div class="search-result-box clear-fix">
      <div class="search-left-part float-left">
        <div class="search-result-list">
          ...
          <div
            ...
            v-if="searchResult.contents.length !== 0"
          >
            <div
              class="search-result-item"
              v-for="(item, index) in searchResult.contents"
              :key="index"
            >
      ...
      <div class="search-right-part float-left">
        <div class="search-hot-word" id="search-hot-word">
          <div class="card-header-g">
            热门标签
          </div>
          <WordCloud></WordCloud>
        </div>
        <div class="search-right-box" id="search-right-subscription-box">
          <div class="card-header-g">
            公众号
          </div>
...
export default {
  ...
  async asyncData({ query }) {
    ...
    //发起请求，获取搜索内容
    let result = await api.getSearchContent(
      categoryId,
      keyword,
      page,
      size,
    ...
    onWindowScroll() {
      ...
      if (weixinBox) {
        ...
        if (scrolledTop >= bottomOfWC) {
          ...
          weixinBox.style.top = "60px";
          weixinBox.style.width = "210px";
          //显示我们的悬浮内容
        } else {
          weixinBox.style.position = "fixed";
          weixinBox.style.top = bottomOfWC - scrolledTop + 12 + "px";
          // console.log('隐藏悬浮内容...')
        }

        //处理左右滑动
        if (scrolledLeft > 0) {
          weixinBox.style.left =
            parentPart.offsetLeft +
            parentPart.offsetWidth -
            weixinBox.offsetWidth -
            scrolledLeft +
            "px";
        } else {
          //正常状态的，左边应该它老爸的左边
